//
//  TranslucentBackgroundStyle.swift
//  MathLateX
//
...
//
//  TraslucentWindowStyle.swift
//  MathLateX
//
//  Created by christian on 10/05/24.
//
...
extension WindowBackgroundStyle where Self == TranslucentBackgroundStyle {
  public static var hiddenTitleBar: TranslucentBackgroundStyle {
    TranslucentBackgroundStyle(config: .translucent)
  }

  public static func hiddenTitleBar(material: NSVisualEffectView.Material)
    -> TranslucentBackgroundStyle
  {
...
extension View {
  ...
  @MainActor public func presentedWindowBackgroundStyle<S>(_ style: S) -> some View
  where S: WindowBackgroundStyle {
  ...
  struct WindowConfiguration {
    ...
    static func configure(window: NSWindow, forConfig config: WindowConfiguration) {
      ...
      window.standardWindowButton(.miniaturizeButton)?.isHidden =
        config.standardWindowButtonConfig.miniaturizeButtonIsHidden
      window.standardWindowButton(.closeButton)?.isHidden =
        config.standardWindowButtonConfig.closeButtonIsHidden
      window.standardWindowButton(.zoomButton)?.isHidden =
        config.standardWindowButtonConfig.zoomButtonIsHidden
    ...
    func createWindowConfigurationForCurrentContext(_ context: NSWindow) -> WindowConfiguration {
      WindowConfiguration(
        ...
        standardWindowButtonConfig: WindowConfiguration.StandardWindowButtonConfiguration(
          miniaturizeButtonIsHidden: context.standardWindowButton(.miniaturizeButton)?.isHidden
            ?? false,
