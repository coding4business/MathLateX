...
struct SettingsData: Codable, Hashable {
  ...
  init(from decoder: Decoder) throws {
    ...
    self.navigation =
      try container.decodeIfPresent(NavigationSettings.self, forKey: .navigation) ?? .init()
    ...
    self.developerSettings =
      try container.decodeIfPresent(
        DeveloperSettings.self, forKey: .developerSettings
      ) ?? .init()
    self.languageSettings =
      try container.decodeIfPresent(
        SettingsData.LanguageSettings.self, forKey: .languageSettings
      ) ?? .init()
  }
  ...
  func propertiesOf(_ name: SettingsPage.Name) -> [SettingsPage] {
    ...
    case .textEditing:
      textEditing.searchKeys.forEach {
        settings.append(.init(name, isSetting: true, settingName: $0))
      }
    case .terminal:
    ...
    case .sourceControl:
      sourceControl.searchKeys.forEach {
        settings.append(.init(name, isSetting: true, settingName: $0))
      }
    case .location:
      LocationsSettings().searchKeys.forEach {
        settings.append(.init(name, isSetting: true, settingName: $0))
      }
    case .developer:
      developerSettings.searchKeys.forEach {
        settings.append(.init(name, isSetting: true, settingName: $0))
      }
    case .language:
      languageSettings.searchKeys.forEach {
        settings.append(.init(name, isSetting: true, settingName: $0))
      }
